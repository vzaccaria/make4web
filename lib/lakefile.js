(function(){
  var _, color, moment, ref$, filter, map, bbo, ita, cl, cd, mprint, pd, mok, merr, pathSystem, recomputePaths, silent, sensitiveDirs, tag, savePid, watch, nodemon, watchWRate, serve, echo, echoStep, echoOk, ct, ce, bt, ht, le, targets, help, npmGitAction, gitAction, makeify, getvar, all, join, changeType, rebaseTo, takeName, takeDir, trim, reduceTo, now, resetTargets, addToTargets, getTargets, o, m, x, p, it, foreachFileIn, foreachFileInExpression, collectTargets, joinTargets, copyTargets, installVariable, installFile, generateMakefileConfigPs, task, pretty, hooksData, hooks, translationPlugins, plugins, generateMakefileExt, join$ = [].join, this$ = this;
      ...
      for (i$ = 0, len$ = fromSourceList.length; i$ < len$; ++i$) {
        ...
        if (src.name != null) {
          if (customType != null && customType && plugins.translationPairs[src.type]) {
            finalType = plugins.translationPairs[src.type];
          }
          src.dstName = changeType(src.type, finalType)(
          takeName(
          src.name));
          src.watchName = src.name;
          src.dirName = takeDir(
          src.name);
        }
        if (src.filesOfType != null) {
          if (customType != null && customType && plugins.translationPairs[src.filesOfType]) {
            finalType = plugins.translationPairs[src.filesOfType];
          }
          src.dstName = takeName(
          changeType(src.filesOfType, finalType)(
          all({
            filesOfType: src.filesOfType,
            'in': src['in']
          })));
          src.watchName = src['in'];
          src.dirName = src['in'];
        }
  ...
  hooksData = (function(){
    ...
    prototype.initHooksData = function(cf, sf, vf, cs, ch, ot){
      var i$, ref$, len$, l, results$ = [];
      for (i$ = 0, len$ = (ref$ = [ot, ch, cs, cf, sf, vf]).length; i$ < len$; ++i$) {
        l = ref$[i$];
  ...
  translationPlugins = (function(){
    ...
    prototype.deployExtensionInto = function(ext, intoDir){
      ...
      return this.hooks.addHook('_deploy', null, function(pathSystem){
        ...
        return x("cp " + pathSystem.buildDir + "/*." + ext + " " + intoDir(pathSystem));
      });
    };
    ...
    prototype.copyExtension = function(ext, intoDir){
      var this$ = this;
      plugins.addTranslation(ext, ext, function(sourceName, destName, depencencies, buildDir){
  ...
  generateMakefileExt = function(pathSystemOptions, files){
    var buildDir, deployDir, serverDir, clientDir, clientDirImg, clientDirFonts, cf, vf, sf, cs, ch, im, fo, opt, ot, triggerFiles, additionalCommands, additionalDependencies, assetsTargets, i$, ref$, len$, path;
    ...
    resetTargets();
    hooks.initHooksData(cf, sf, vf, cs, ch, ot);
    collectTargets({
      fromSourceList: ot,
    ...
    it('deploy files', {
      ...
      if ((opt != null ? opt.minifyJs : void 8) != null) {
        installFile({
    ...
    if ((opt != null ? opt.minifyJs : void 8) != null) {
      additionalDependencies = buildDir + "/client.min.js " + buildDir + "/vendor.min.js";
    }
    if ((opt != null ? opt.minifyCss : void 8) != null) {
    ...
    assetsTargets = (im != null ? getvar('client img') : void 8) + " " + (im != null ? getvar('client fonts') : void 8) + " ";
    task({
      withName: "deploy",
